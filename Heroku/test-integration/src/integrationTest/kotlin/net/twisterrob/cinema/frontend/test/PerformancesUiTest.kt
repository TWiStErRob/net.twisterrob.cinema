...
class PerformancesUiTest {
	...
	@Nested
	inner class `Performances display` {

		@Nested
		inner class `performances table by film` {
			...
			@Test fun `shows performances (Spider-Man)`() {
				val times =
					app.performances.byFilm.performances("(IMAX 3-D) Spider-Man : HOMECOMING", "Leicester Square")
				assertThat(times).hasSize(4)
		...
		@Nested
		inner class `performances table by cinema` {
