...
public abstract class BaseListRequest<T extends CineworldBase> {
	...
	/**
	 * @optional GB
	 * @return Sets which territory to return cinemas for, valid values for United Kingdom
	 *         and Ireland are; GB and IE.
	 */
	...
	/**
	 * @optional GB
	 * @param territory Sets which territory to return cinemas for, valid values for
	 *            United Kingdom and Ireland are; GB and IE.
	 */
	...
	/**
	 * @optional N/A
	 * @return Wraps the response JSON in the callback function specified to allow cross
	 *         browser scripting, note that if you use jQuery and JSONP the callback
	...
	/**
	 * @optional N/A
	 * @param callback Wraps the response JSON in the callback function specified to allow
	 *            cross browser scripting, note that if you use jQuery and JSONP the
	...
	/**
	 * Return the request type of this request, for display purposes; but return an
	 * identifier-like {@link String}.
	 * 
	 * @return request type
	 */
	...
	protected URL makeUrl(final String requestType, final Object... filters) {
		...
		try {
			...
			LOG.error("Cannot initialize urls: %s (%s -> %s)", ex, requestType,
					Arrays.toString(filters), spec);
			return BASE_URL; // TOD throw
		}
	}

	public URL makeUrl(final String requestType, final String... filters) {
		StringBuilder filterString = new StringBuilder();
		...
		try {
			String spec = String.format("%s?key=%s%s", requestType, DEFAULT_DEVELOPER_KEY,
					filterString);
			return new URL(BASE_URL, spec);
		} catch (MalformedURLException ex) {
			LOG.error("Cannot initialize urls: %s (%s)", ex, requestType, Arrays.toString(filters));
			return BASE_URL; // TOD throw
		}
	}
}
