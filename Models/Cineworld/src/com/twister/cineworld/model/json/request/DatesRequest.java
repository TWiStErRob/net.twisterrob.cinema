...
public class DatesRequest extends BaseListRequest<CineworldDate> {
	...
	/**
	 * @param cinema (Optional, no default) Cinema ID to retrieve dates for, or for all
	 *            cinemas if excluded. More than one cinema id can be passed in, by
	 *            assigning multiple values: cinema=1&cinema=2 - this will mean dates are
	 *            retrieved on which there are performances at at least ONE of the
	 *            cinemas.
	 * @param film (Optional, no default) Film EDI to retrieve dates for, or for all films
	 *            if excluded. More than one edi can be passed in, by assigning multiple
	 *            values: film=1234&film=2345 - this will mean dates are retrieved that
	 *            match at least ONE edi.
	 * @param category (Optional, no default) Category code- this can be passed in to
	 *            filter the results to only contain dates on which there is at least on
	 *            performance for a film in this category.
	 * @param event (Optional, no default) Event code- this can be passed in to filter the
	 *            results to only contain dates on which there is at least on performance
	 *            for a film linked to this event.
	 ...
	 */
	public DatesRequest(final List<Integer> cinema, final List<Integer> film,
			final List<String> category, final List<String> event, final List<Integer> distributor) {
		super();
		m_cinema = cinema;
		m_film = film;
		m_category = category;
	 ...
	 *            performance for a film from this distributor.
	 */
	public DatesRequest(final String key, final String territory, final String callback,
			final List<Integer> cinema, final List<Integer> film, final List<String> category,
			final List<String> event, final List<Integer> distributor) {
	...
	@Override public URL getURL() {
		return makeUrl(REQUEST_URI, KEY_CINEMA, m_cinema, KEY_FILM, m_film, KEY_CATEGORY,
				m_category, KEY_EVENT, m_event, KEY_DISTRIBUTOR, m_distributor);
	}
	 ...
	 * @see DatesResponse
	 */
	@Override public Class<DatesResponse> getResponseClass() {
		return DatesResponse.class;
	}

	/**
	 * @optional N/A
	 * @return Cinema ID to retrieve dates for, or for all cinemas if excluded. More than
	 *         one cinema id can be passed in, by assigning multiple values:
	 *         cinema=1&cinema=2 - this will mean dates are retrieved on which there are
	...
	/**
	 * @optional N/A
	 * @param cinema Cinema ID to retrieve dates for, or for all cinemas if excluded. More
	 *            than one cinema id can be passed in, by assigning multiple values:
	 *            cinema=1&cinema=2 - this will mean dates are retrieved on which there
	...
	/**
	 * @optional N/A
	 * @return Film EDI to retrieve dates for, or for all films if excluded. More than one
	 *         edi can be passed in, by assigning multiple values: film=1234&film=2345 -
	 *         this will mean dates are retrieved that match at least ONE edi.
	 * @see CineworldFilm
	 */
	...
	/**
	 * @optional N/A
	 * @param film Film EDI to retrieve dates for, or for all films if excluded. More than
	 *            one edi can be passed in, by assigning multiple values:
	 *            film=1234&film=2345 - this will mean dates are retrieved that match at
	 *            least ONE edi.
	 * @see CineworldFilm
	 */
	 ...
	 *         dates on which there is at least on performance for a film in this
	 *         category.
	 ...
	 *            contain dates on which there is at least on performance for a film in
	 *            this category.
	 * @see CineworldCategory
	 */
	 ...
	 *         dates on which there is at least on performance for a film linked to this
	 *         event.
	 * @see CineworldEvent
	 */
	 ...
	 *            contain dates on which there is at least on performance for a film
	 ...
	 *         dates on which there is at least on performance for a film from this
	 *         distributor.
	 ...
	 *            only contain dates on which there is at least on performance for a film
